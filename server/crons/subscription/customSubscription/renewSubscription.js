...
const RenewAllCustomSubscriptions = async () => {
  try {
    // const timezone = "America/New_York";
    // const nowInTimeZone = moment().tz(timezone);
    // const sixAMToday = nowInTimeZone.clone().startOf("day").add(6, "hours");
    ...
    const query = {
      ...
      // endDate: {
      //   $gte: sixAMToday.toDate(),
      //   $lt: todayStart.add(1, "day").toDate(),
      // },
    };
    ...
    const promises = records.map(async (subscription) => {
      const timeZone = (subscription, "placeId.timeZoneId", "UTC");
      const nowInTimeZone = moment().tz(timeZone);
      const sixAMTodayInTZ = nowInTimeZone
        .clone()
        .startOf("day")
        .add(6, "hours");

      if (moment(subscription.endDate).tz(timeZone).isBefore(sixAMTodayInTZ)) {
        console.log(
          `Renewal Process start for ${subscription.subscriptionNumber}`
        );
        const {
          baseRate,
          customerId,
        ...
        const paymentMethod =
          defaultPaymentMethodId || paymentId.paymentMethodId;
        ...
        if (!paymentIntent.success) {
          const transactionDate = moment
            .unix(
              paymentIntent.data?.payment_intent?.created || moment().unix()
            )
            .utc()
...
module.exports = RenewAllCustomSubscriptions;
