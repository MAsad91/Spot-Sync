...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const paymentData = {
      ...
      description: description || "Subscription payment via EasyPaisa",
      customerEmail: subscription.customerId.email,
      customerMobile: subscription.customerId.mobile,
      txnRefNo: transactionId,
      returnUrl: returnUrl || `${process.env.FRONT_DOMAIN}payment/easypaisa/callback`
    };

    const paymentResult = await easyPaisa.createPaymentRequest(paymentData);

    if (!paymentResult.success) {
      return res.status(http400).json({
        success: false,
        message: paymentResult.message || "Failed to create EasyPaisa payment request",
      });
    }
    ...
    const paymentObject = {
      ...
      transactionDate: new Date(),
      paymentInfo: {
        easyPaisaRequestData: paymentResult.requestData,
    ...
    return res.status(http200).json({
      success: true,
      message: "EasyPaisa payment request created successfully",
      data: {
        transactionId: transactionId,
        paymentUrl: paymentResult.data.pp_RedirectURL || paymentResult.data.redirect_url,
        paymentId: payment._id,
