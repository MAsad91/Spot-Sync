    ...
    try {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            if (placeId === 'all' && isSuperAdmin) {
                ...
                if (globalStats && globalStats.data.length > 0) {
                    ...
                    dayBookings = dayData ? dayData.bookings : 0;
                }
            } else {
        ...
        if (placeId === 'all' && isSuperAdmin) {
            ...
            if (globalStats && globalStats.data.length > 0) {
                ...
                const currentWeekBookings = currentWeekData.reduce((sum, dp) => sum + dp.bookings, 0);
                const previousWeekBookings = previousWeekData.reduce((sum, dp) => sum + dp.bookings, 0);
                
                if (previousWeekBookings > 0) {
                    percentageIncrease = ((currentWeekBookings - previousWeekBookings) / previousWeekBookings * 100).toFixed(2);
                }
            }
        } else {
            const statsDocs = await Statistics.find({ placeId: { $in: places } });
            ...
            for (const statsDoc of statsDocs) {
                if (statsDoc.data.length > 0) {
                    ...
                    currentWeekTotal += currentWeekData.reduce((sum, dp) => sum + dp.bookings, 0);
                    previousWeekTotal += previousWeekData.reduce((sum, dp) => sum + dp.bookings, 0);
                }
            }
