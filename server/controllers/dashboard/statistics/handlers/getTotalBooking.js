    ...
    try {
        ...
        if (placeId === 'all' && isSuperAdmin) {
            ...
            if (globalStats && globalStats.data.length > 0) {
                // Calculate total bookings from all data points
                totalBookings = globalStats.data.reduce((sum, dataPoint) => sum + dataPoint.bookings, 0);
                ...
                const currentMonthBookings = currentMonthData.reduce((sum, dp) => sum + dp.bookings, 0);
                const previousMonthBookings = previousMonthData.reduce((sum, dp) => sum + dp.bookings, 0);
                
                if (previousMonthBookings > 0) {
                    totalPercentageIncrease = ((currentMonthBookings - previousMonthBookings) / previousMonthBookings * 100).toFixed(2);
                }
                statsCount = 1;
            }
        } else {
            ...
            for (const statsDoc of statsDocs) {
                if (statsDoc.data.length > 0) {
                    // Calculate total bookings from all data points
                    const placeBookings = statsDoc.data.reduce((sum, dataPoint) => sum + dataPoint.bookings, 0);
                    ...
                    if (previousMonthBookings > 0) {
                        const placePercentageIncrease = ((currentMonthBookings - previousMonthBookings) / previousMonthBookings * 100);
