        ...
        for (let i = 11; i >= 0; i--) {
            ...
            if (placeId === 'all' && isSuperAdmin) {
                ...
                if (globalStats && globalStats.data.length > 0) {
                    ...
                    monthBookings = monthData.reduce((sum, dp) => sum + dp.bookings, 0);
                }
            } else {
        ...
        if (placeId === 'all' && isSuperAdmin) {
            ...
            if (globalStats && globalStats.data.length > 0) {
                ...
                if (previousMonthBookings > 0) {
                    percentageIncrease = ((currentMonthBookings - previousMonthBookings) / previousMonthBookings * 100).toFixed(2);
                }
            }
        } else {
            const statsDocs = await Statistics.find({ placeId: { $in: places } });
            ...
            for (const statsDoc of statsDocs) {
                if (statsDoc.data.length > 0) {
                    ...
                    currentMonthTotal += currentMonthData.reduce((sum, dp) => sum + dp.bookings, 0);
                    previousMonthTotal += previousMonthData.reduce((sum, dp) => sum + dp.bookings, 0);
                }
            }
