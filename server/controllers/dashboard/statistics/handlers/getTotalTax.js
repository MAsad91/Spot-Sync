    ...
    try {
        ...
        if (placeId === 'all' && isSuperAdmin) {
            ...
            if (globalStats && globalStats.data.length > 0) {
                // Calculate total tax from all data points
                totalTax = globalStats.data.reduce((sum, dataPoint) => sum + dataPoint.tax, 0);
                ...
                const currentMonthTax = currentMonthData.reduce((sum, dp) => sum + dp.tax, 0);
                const previousMonthTax = previousMonthData.reduce((sum, dp) => sum + dp.tax, 0);
                
                if (previousMonthTax > 0) {
                    totalPercentageIncrease = ((currentMonthTax - previousMonthTax) / previousMonthTax * 100).toFixed(2);
                }
                statsCount = 1;
            }
        } else {
            ...
            for (const statsDoc of statsDocs) {
                if (statsDoc.data.length > 0) {
                    // Calculate total tax from all data points
                    const placeTax = statsDoc.data.reduce((sum, dataPoint) => sum + dataPoint.tax, 0);
                    totalTax += placeTax;
                    ...
                    if (previousMonthTax > 0) {
                        const placePercentageIncrease = ((currentMonthTax - previousMonthTax) / previousMonthTax * 100);
