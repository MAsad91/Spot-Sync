...
const StripePaymentTypeCard = (props) => {
  ...
  useEffect(() => {
    if (!isEmpty(selectedPlace)) {
      ...
      formikRef?.current?.setFieldValue(
        "isDirectChargeLocation",
        get(selectedPlace, "isDirectChargeLocation", false)
      );
    }
  }, [selectedPlace]);

  const handelPaymentTypeChange = (rest) => {
    if (selectedPlace.paymentGatewayFeePayBy === "isbp") {
      setNotification({
        ...notification,
        color: "error",
        title: "Error",
        content: "Payment Gateway Fee Pay By is ISBParking, You can't change it.",
        icon: "warning",
        show: true,
      });
      ...
      <Card sx={{ boxShadow: 3 }}>
        <MDBox p={2}>
          <Formik
            innerRef={formikRef}
            initialValues={{
              isDirectChargeLocation: false
            }}
            onSubmit={(value, action) => {
              console.log("values", value);
              handelPaymentTypeChange(value);
            }}
            enableReinitialize
          >
            {(props) => (
                    ...
                    <Grid container alignItems="center" spacing={1}>
                      ...
                      <Grid item>
                        <Switch
                          ...
                          checked={props.values.isDirectChargeLocation}
                          onChange={(event) =>
                            props.setFieldValue(
                              "isDirectChargeLocation",
...
export default StripePaymentTypeCard;
