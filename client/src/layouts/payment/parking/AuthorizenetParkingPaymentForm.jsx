...
const AuthorizenetParkingPaymentForm = () => {
  ...
  async function getPaymentMethods(shortlyData) {
    try {
      if (shortlyData?.customerId) {
        const paymentMethods = await dispatch(getAuthorizenetPaymentMethods({
          customerId: shortlyData.customerId, 
          placeId: shortlyData.placeId._id
        })).unwrap();
	...
	async function handleGetShortlyData() {
		try {
			...
			if (res?.shortlyData?.customerId) {
				getPaymentMethods(res.shortlyData)
			}
	...
	const handleCardSubmit = async (card) => {
		...
		try {
			const paymentData = {
				...
				shortlyId: shortlyData._id,
        customerId: shortlyData.customerId,
				totalAmount: totalPayAmount
			};

			const serverResponse = await axios.post(
				`${process.env.REACT_APP_API_END_POINT}/payment/authorizenet/makeParkingPayment`,
				paymentData
			);
	...
	const totalPayAmount = get(shortlyData, "totalAmount", 0);
																	...
																	<MDBox>
																		{paymentProfiles.map((card) => (
																			<AuthorizenetCard
																				...
																				getPaymentMethods={() => getPaymentMethods(shortlyData)}
																			/>
																		))}
																	</MDBox>
...
export default AuthorizenetParkingPaymentForm;
