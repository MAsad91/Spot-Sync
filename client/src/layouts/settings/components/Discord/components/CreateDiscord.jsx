...
import { createDiscord } from "store/slice/discord/discordSlice";

const CreateDiscord = (props) => {
...
const placesData = useSelector((state) => state.places?.selectedPlace);
  let placeId = placesData?._id || new URLSearchParams(location?.search).get("placeId");
  if (!placeId) {
    placeId = localStorage.getItem("placeId");
  }

  const handleCreateDiscord = (discordData) => {
    console.log("discordData ===>", discordData);
    setIsLoading(true);
    dispatch(createDiscord(discordData))
      .unwrap()
      .then((res) => {
        console.log("discord creation response:", res);
      ...
      .catch((err) => {
        console.error("Error creating Discord:", err);
        setIsLoading(false);
        setNotification({
          ...notification,
          color: "error",
